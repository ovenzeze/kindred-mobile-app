...
**当前（2026-06-04 复核）：** `kapi.deth.us`（本地 3102）OpenAPI 已含 **34** 条 path，含 `albums` / `profileFields`。`index.ts` 已注册对应 router。验证：`curl -s https://kapi.deth.us/api/openapi.json | jq '.paths | keys | length'`（期望 34）。
