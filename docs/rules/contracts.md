...
后端仓库：`kindred-api-server`。契约优先原则以后端 `src/contracts/*.ts` 为准；前端变更应跟随后端 `src/contracts/` 或已部署的 `/api/openapi.json`。
...
**当前（2026-06-04 复核）：** `kapi.deth.dev`（本地 3102）OpenAPI 已含 **34** 条 path，含 `albums` / `profileFields`。`index.ts` 已注册对应 router。验证：`curl -s https://kapi.deth.dev/api/openapi.json | jq '.paths | keys | length'`（期望 34）。
