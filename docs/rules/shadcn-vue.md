...
- 字体在 `app/assets/css/main.css` 顶部通过 Google Fonts `@import` 加载 Geist；`@theme inline` 中 `--font-sans` 引用该族名。标题、正文、控件统一使用 Geist，避免页面局部混用字体。`@nuxt/fonts` 模块已注册但当前不单独配置 font family。
