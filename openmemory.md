...
- **规则与路径：** `CLAUDE.md`、`docs/rules/{frontend,contracts,shadcn-vue,agent-browser-qa}.md` 中引用的关键文件与命令均复核存在。
- **页面与 Sitemap：** `app/pages/` 当前包含首页、matches、profile、chat index/detail、auth login/register/callback、ui-kit；`scripts/generate-sitemap.ts` 仍覆盖 7 个公开入口，动态单聊页与 OAuth callback 不纳入 sitemap。
- **Sitemap：** `npm run sitemap` 已刷新 `public/sitemap.xml`（7 URLs，`lastmod=2026-08-05`）。
- **代码状态：** `preview` 分支自 2026-07-22 维护提交后无新增业务提交；文档规则无需结构性修正。
...
- **规则与路径：** `CLAUDE.md`、`docs/rules/{frontend,contracts,shadcn-vue,agent-browser-qa}.md` 中引用的关键文件与命令均复核存在。
- **页面与 Sitemap：** `app/pages/` 当前包含首页、matches、profile、chat index/detail、auth login/register/callback、ui-kit；`scripts/generate-sitemap.ts` 仍覆盖 7 个公开入口，动态单聊页与 OAuth callback 不纳入 sitemap。
- **Sitemap：** `npm run sitemap` 已刷新 `public/sitemap.xml`（7 URLs，`lastmod=2026-08-09`）。
- **代码状态：** `preview` 分支自 2026-08-05 维护提交后无新增业务提交；现有组件、store、middleware、utils 与 2026-06-04/05 记录一致。
